...
foreach ($esxi in $hosts)
{
    ...
    if($singleAdapter -eq $true){
        ...
        $networkAdapter = $esxi | Get-VMHostNetworkAdapter -Physical -Name vmnic3 
        Remove-VirtualSwitchPhysicalNetworkAdapter -VMHostNetworkAdapter $networkAdapter -Confirm:$false
        #Add Host to DVS
        $dvs1 | Add-VDSwitchVMHost -VMHost $esxi
        #Add NIC to DVS
        ...
        $networkAdapter = $esxi | Get-VMHostNetworkAdapter -Physical -Name vmnic5 
        Remove-VirtualSwitchPhysicalNetworkAdapter -VMHostNetworkAdapter $networkAdapter -Confirm:$false
        #Add Host to DVS
        $dvs2 | Add-VDSwitchVMHost -VMHost $esxi
        #Add NIC to DVS
...
$results | Export-Csv -Path .\Connectivity_test1_$cluster.csv